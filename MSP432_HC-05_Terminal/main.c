 ...
 *
 * Command For Slave
 * 8. AT+ADDR? (Get Address), 98d3,51,fd9017
 *
 * Command For Master
 * 9. AT+ROLE1 (Config to Master)
 * 10.AT+CMODE=0 (Connect to one)
 * 11.AT+BIND=98d3,51,fd9017 (Connect Master to Slave)
 *
 * To verfiy if configuration is correct
 * 12.AT+ROLE? (1 = Master, 0 = Slave)
 * 13.AT+CMODE? (Show mode 0 = One, 1 = many)
 * 14.AT+BIND? (Show address to bind)
 */
